...
from harness.loop.parser import clean_final_text
...
def mount(app: FastAPI) -> None:
    ...
    async def inspect(msg: InspectMessage):
        ...
        client = _get_client(msg.model)
        await _maybe_resolve_model(client, msg.model)  # BF-011: same resolution as chat
        sources = [s for s in msg.sources if isinstance(s, dict)][:5]
        try:
            raw = await asyncio.to_thread(client.chat, _inspect_transcript(msg, sources))
        except LLMConnectionError as exc:
            raise HTTPException(503, detail=f"Cannot reach the AI endpoint: {exc}") from exc
        except LLMResponseError as exc:
            raise HTTPException(502, detail=str(exc)) from exc
        final = clean_final_text(raw)
        if not final:
